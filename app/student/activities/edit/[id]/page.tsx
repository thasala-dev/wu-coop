...
export default function EditActivity({ params }: { params: { id: string } }) {
  ...
  const deleteFile = async (fileId: number) => {
    try {
      const response = await fetch(`/api/student-activities/${activityId}/files/${fileId}`, {
        method: "DELETE",
      });
  ...
  const uploadFiles = async (files: File[]) => {
    for (const file of files) {
      ...
      try {
        const response = await fetch(`/api/student-activities/${activityId}/upload`, {
          method: "POST",
          body: formData,
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate form
    if (!formData.activityDate || !formData.categoryId || !formData.title || !formData.description) {
      toast({
        title: "กรุณากรอกข้อมูลให้ครบถ้วน",
        description: "กรุณากรอกข้อมูลที่จำเป็น: วันที่, ประเภทกิจกรรม, หัวข้อ และรายละเอียด",
        variant: "destructive",
      });
                    ...
                    <form className="space-y-6" onSubmit={handleSubmit}>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                          <Label htmlFor="activity-activityDate">วันที่ทำกิจกรรม</Label>
                      ...
                      {formData.newFiles.length > 0 && (
                        <div className="space-y-2">
                          ...
                          <div className="space-y-2">
                            {Array.from(formData.newFiles).map((file, index) => (
                          ...
                          <div className="space-y-2">
                            {files.map((file) => (
                              <div key={file.id} className="flex items-center justify-between p-2 border rounded-md">
                                <span>{file.originalFilename} ({formatFileSize(file.fileSize)})</span>
                                <AlertDialog>
