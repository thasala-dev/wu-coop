  ...
  const filterEvents = () => {
    ...
    if (calendarSelected) {
      filtered = filtered.filter((event) =>
        event.calendar_id === parseInt(calendarSelected)
      );
    }
  ...
  const formatEventData = (apiEvents: any[]) => {
    return apiEvents.map((event) => {
      ...
      return {
        ...
        term: event.calendar_id && calendars.length > 0 
                  ...
                  <Tabs defaultValue="list">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                      ...
                      <div className="flex gap-2 w-full md:w-auto">
                        ...
                        <Select
                          defaultValue={calendarSelected ? calendarSelected.toString() : ""}
                          onValueChange={(value) => {
                                  ...
                                  <div className="space-y-1">
                                    {day.events.slice(0, 2).map((event) => (
                                      <div 
                                        ...
                                        className={`p-1 text-xs rounded truncate flex items-center ${
                                          event.category === "สำคัญ" ? "bg-red-100 text-red-800" :
                                          event.category === "กำหนดส่ง" ? "bg-blue-100 text-blue-800" :
                                          event.category === "การนิเทศ" ? "bg-purple-100 text-purple-800" :
                                          event.category === "การนำเสนอ" ? "bg-green-100 text-green-800" :
                                    ...
                                    {day.events.length > 2 && (
                                      <div className="text-xs text-gray-500 pl-1">
                                        + {day.events.length - 2} กิจกรรมเพิ่มเติม
                                      </div>
                                    )}
                                  </div>
                                </>
