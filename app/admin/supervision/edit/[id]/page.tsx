...
export default function EditSupervision() {
  ...
  const fetchSupervisionData = async () => {
    ...
    try {
      ...
      // จำลองการดึงข้อมูล
      setTimeout(() => {
        setValue("registInternId", mockSupervision.regist_intern_id);
        setValue("advisorId", mockSupervision.advisor_id);
        setValue("scheduledDate", mockSupervision.scheduled_date);
        setValue("status", mockSupervision.status);

        if (mockSupervision.scheduled_date) {
          setScheduledDate(
            parse(mockSupervision.scheduled_date, "yyyy-MM-dd", new Date())
          );
        }
